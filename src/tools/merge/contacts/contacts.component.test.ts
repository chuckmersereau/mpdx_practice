...
describe('tools.mergeContacts.component', () => {
    let $ctrl, rootScope, scope, componentController, alerts, gettextCatalog, state, api, contacts, tools, q;
    beforeEach(() => {
        angular.mock.module(component);
        inject((
            $componentController, $rootScope, $state, _alerts_, _api_, _gettextCatalog_, _contacts_, _tools_, $q
        ) => {
            rootScope = $rootScope;
            scope = rootScope.$new();
            alerts = _alerts_;
    ...
    describe('load', () => {
        ...
        it('should map data', (done) => {
            let data: any = [{ id: 1, records: [{ id: 'contact_1_id' }, { id: 'contact_2_id' }] }];
