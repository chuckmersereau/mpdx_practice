...
describe('common.acceptInvite.component', () => {
    let $ctrl, componentController, scope, rootScope, state, gettextCatalog, alerts, api, q;

    beforeEach(() => {
        angular.mock.module(component);
        inject(($componentController, $rootScope, $state, _gettextCatalog_, _alerts_, _api_, $q) => {
    ...
    describe('$onInit', () => {
        ...
        describe('promise rejected', () => {
            it('should call state.go', (done) => {
                spyOn(api, 'put').and.callFake(() => q.reject({}));
                $ctrl.$onInit().catch(() => {
                    expect(state.go).toHaveBeenCalledWith('home');
                    done();
                });
